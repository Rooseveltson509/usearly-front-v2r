const staticLogos: Record<string, string> = {
  ...
  mistral: "https://upload.wikimedia.org/wikipedia/commons/4/4b/Mistral_logo.svg",
};

const fallbackClearbit = (domain: string) => `https://logo.clearbit.com/${domain}`;
const fallbackFavicon = (domain: string) => `https://www.google.com/s2/favicons?domain=${domain}&sz=128`;
...
export const FALLBACK_BRAND_PLACEHOLDER = `data:image/svg+xml,${encodeURIComponent(placeholderSvg)}`;
...
function normalizeDomain(siteUrl: string): string {
  return siteUrl.replace(/^https?:\/\//, "").replace(/^www\./, "").split("/")[0].trim();
}
...
async function resolveBrandLogo(brand: string, siteUrl?: string): Promise<string> {
  const key = brand.toLowerCase().trim();
