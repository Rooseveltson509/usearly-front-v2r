...
function makeDefaultRules(
  ...
  return {
    ...
    special: {
      ...
      test: (value) =>
        /[^A-Za-z0-9]/.test(value) && !invalidSpecial.test(value),
    },
  };
}
...
export default function PasswordRules({
  ...
  const entries = useMemo(
    () =>
      (Object.entries(merged) as [RuleKey, Rule][]).filter(
        ([k]) => !enabled || enabled.includes(k),
      ),
    [merged, enabled],
  );
  ...
  const allValid = entries.every(([, rule]) => rule.test(value));
    ...
    <div className={className}>
      ...
      {disallowed && (
        <p className="invalid-special">
          ...
          <strong>{allowedSpecialChars.split("").join(" ")}</strong> sont
          permis.
        </p>
      )}
    </div>
