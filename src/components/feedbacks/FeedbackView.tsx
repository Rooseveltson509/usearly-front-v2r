...
import { groupByBrandThenCategory, groupByBrand, explodeGroupedReports } from "@src/utils/feedbackListUtils";
...
const FeedbackView = ({
    ...
    if (activeTab === "report") {
        if (viewMode === "grouped") {
            const groupedByBrand = groupByBrandThenCategory(groupByBrand(explodeGroupedReports(currentState.data)));
            return (
                <GroupedReportList
        ...
        if (viewMode === "flat") {
            const groupedFlat = Object.entries(groupByBrand(explodeGroupedReports(currentState.data)));
            return (
    ...
    return (
        <>
            {currentState.data.map((item, index) => (
                <InteractiveFeedbackCard key={item.id || `feedback-${index}`} item={{ ...item, type: activeTab }} />
            ))}
        </>
    );
};
