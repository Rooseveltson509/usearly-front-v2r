...
function InputTextAccount({ id, label, type = "text", value, onChange, disabled = false }: InputTextAccountProps) {
    const minWidth = 50;
    const maxWidth = 640;
    ...
    return(
        <div className="input-text-account">
            ...
            <div>
                ...
                <input ref={inputRef} type={type} id={id} value={value} onChange={onChange} disabled={disabled} placeholder={label} style={{ boxSizing: "content-box", width: type === "date" ? "100%" : width }} />
                { !disabled && <img src={PenModifyIcon} alt="Modifier" className="modify-icon" /> }
            </div>
        </div>
    )
}
