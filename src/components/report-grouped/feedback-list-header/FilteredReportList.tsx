...
import FilteredDescriptionCard from "./FilteredDescriptionCard";
//import "./FilteredReportList.scss";
import { getFilteredReportDescriptions } from "@src/services/feedbackService";
...
const FilteredReportList = ({ brand, category }: Props) => {
  const [descriptions, setDescriptions] = useState<FeedbackDescription[]>([]);
  const [page, setPage] = useState(1);
  const [hasMore, setHasMore] = useState(true);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    // Reset quand on change les filtres
    setDescriptions([]);
    setPage(1);
    setHasMore(true);
  }, [brand, category]);
  ...
  return (
    <div className="filtered-report-list" onScroll={handleScroll}>
      {descriptions.map((desc) => (
        <FilteredDescriptionCard key={desc.id} description={desc} />
      ))}
      {loading && <p className="loading">Chargement...</p>}
      {!hasMore && !loading && descriptions.length > 0 && (
        <p className="end-message">Fin des résultats</p>
