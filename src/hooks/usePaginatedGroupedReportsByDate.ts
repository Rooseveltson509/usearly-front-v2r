...
export const usePaginatedGroupedReportsByDate = (enabled: boolean) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                setLoading(true);
                const response: GetGroupedReportsByDateResponse = await getGroupedReportsByDate(page, 20);
                ...
                console.error("Erreur lors du chargement des reports groupés par date :", error);
                setHasMore(false);
            } finally {
                setLoading(false);
            }
