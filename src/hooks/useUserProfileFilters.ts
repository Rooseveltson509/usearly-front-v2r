...
export const useUserProfileFilters = (data: UserGroupedReport[]) => {
  ...
  const availableCategories = useMemo(() => {
    const categories = Array.from(new Set(data.map(d => d.subCategory))).sort();
    return ["Tous", ...categories];
  }, [data]);

  const filteredData = useMemo(() => {
    return data.filter(item => {
      const brandMatch = selectedBrand === "Tous" || item.marque === selectedBrand;
      const categoryMatch = selectedCategory === "Tous" || item.subCategory === selectedCategory;
